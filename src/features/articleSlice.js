...
import {fetchArticles,addArticle,deleteArticle,editArticle,fetchArticleById, fetchArticlesPagesServ} from
...
export const getArticlespages= createAsyncThunk(
    ...
    try {
    const res = await 
    fetchArticlesPagesServ(mesparams.currentPage,mesparams.itemsPerPage);
...
.addCase(createArticle.pending, (state, action) => {
    ...
    state.articles=[action.payload,...state.articles] //bech ki nzid articles yji meloul
    state.isLoading=false;
    state.error=null;
